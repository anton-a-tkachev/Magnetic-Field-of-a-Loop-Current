...
Br = lambda r,z,a,i: mu0*i/2/np.pi*z/r/np.sqrt((a + r)**2 + z**2)*(-ellipk(k(r,z,a)) + (a**2 + r**2 + z**2)/((a - r)**2 + z**2)*ellipe(k(r,z,a))) if i != 0.0 and r != 0.0 else 0.0

# Azimuthal vector potential
Ap = lambda r,z,a,i: mu0*i/np.pi*a/np.sqrt((r + a)**2 + z**2)*((2 - k(r,z,a))*ellipk(k(r,z,a)) - 2*ellipe(k(r,z,a)))/k(r,z,a) #if i != 0.0 and r != 0.0 else 0.0
...
def AA(R, Z, ID, OD, DZ, W, I, NR, NZ):
    ...
    RR = np.linspace(0.5*ID, 0.5*OD, NR)
    PP = np.array([[rr,zz] for rr in RR for zz in ZZ])
    AA = np.sum(np.array([B(R, Z - pp[1], pp[0], I*W/NR/NZ) for pp in PP]), axis=0)
    return AA
